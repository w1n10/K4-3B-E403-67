...
- **Lát cắt MỘT CÂU:**  
  *Một học viên vừa học xong buổi 6 · dạy lại khái niệm "Vì sao LLM bịa" cho bạn học Agent · AI đánh giá mức độ bao phủ kiến thức và hỏi ngược đúng chỗ còn hổng mà tuyệt đối không lộ đáp án · học viên giải thích bổ sung thành công đạt độ phủ ≥ 5/7.*
...
- **Golden set (20 case trong `golden/cases.json`):**
  - $\ge 8$ case trích từ chatlog thật trong `vlearn.db` (`T01922`, `T00842`, `T00207`...).
  - $8$ case bao phủ 4 lớp chỗ khó (theo bảng §5).
  - $4$ case paraphrase bằng ngôn ngữ tự nhiên/đời thường.
...
- **Phùng Quang Minh Huy:** Khai phá dữ liệu chatlog `vlearn.db`, tạo `content/llm-hallucination.json`, trích xuất evidence chuẩn B.
- **Lưu Nguyên Khôi:** Xây dựng bộ test Golden Set 20 case, kiểm thử độ nhạy tiêu chí "đủ căn cứ".
- **Chu Thùy Dương:** Xây dựng giao diện Next.js, Route handler điều phối 3 stage, adapter `lib/llm.ts`.
- **Nguyễn Minh Hiếu:** Hoàn thiện `spec.md`, triển khai `lib/guard.ts`, `lib/evaluator.ts`, `lib/persona.ts`, chạy eval và báo cáo.
...
| 18/09 - 09:15 | Tách checklist thành 7 tiêu chí K1-K7 và 3 Misconceptions M1-M3 có gắn `evidence_turn_ids` thật | Đồng bộ hóa dữ liệu khai phá từ `vlearn.db` vào `content/llm-hallucination.json` |
